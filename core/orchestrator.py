...
class AgentOrchestrator:
    ...
    def run(self, goal: str) -> Task:
        ...
        task.transition(TaskState.PLANNING, reason="orchestrator started planning")
        ...
        if not response.tool_calls:
            ...
            task.transition(TaskState.EXECUTING, reason="no tool call, direct answer")
            task.transition(TaskState.VERIFYING, reason="trivial: text answer needs no verification")
            task.transition(TaskState.DONE, reason=response.text)
            return task
        ...
        if decision.decision == PolicyDecision.DENY:
            task.transition(TaskState.BLOCKED, reason=decision.reason)
            return task

        if decision.decision in (PolicyDecision.REQUIRE_CONFIRMATION, PolicyDecision.REQUIRE_SECOND_FACTOR):
            task.transition(TaskState.AWAITING_CONFIRMATION, reason=decision.reason)
            # Real UI/voice confirmation hook goes here in a later sprint.
            return task

        # ALLOW path
        task.transition(TaskState.EXECUTING, reason=f"running tool '{tool_name}'")
        tool = self.tools.get(tool_name)
        try:
            result = tool.run(**tool_args)
        except Exception as exc:  # noqa: BLE001 - deliberately broad, becomes a FAILED state
            task.transition(TaskState.FAILED, reason=str(exc))
            return task

        task.transition(TaskState.VERIFYING, reason="checking tool result")
        # Verification is a stub for now — a real check belongs per-tool later.
        verified = result is not None
        if verified:
            task.transition(TaskState.DONE, reason=str(result))
        else:
            task.transition(TaskState.FAILED, reason="tool returned no result")
